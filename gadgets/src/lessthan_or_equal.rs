...
pub trait LtEqInstruction<F: Field> {
...
impl<F: Field, const N_BYTES: usize> LtEqChip<F, N_BYTES> {
    ...
    pub fn configure(
        ...
        meta.create_gate("lt gate", |meta| {
            ...
            let check_a =
                lhs(meta) - (rhs(meta) + one_expression)- expr_from_bytes(&diff_bytes) + (lt.clone() * range);
...
mod test {
    ...
    fn row_diff_is_lt() {
        ...
        // ok
        try_test_circuit!(vec![1, 1, 1, 1, 1], vec![true, true, true, true], Ok(()));
